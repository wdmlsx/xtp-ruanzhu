 ...
 * */
@Describe(
  "R-MIRROR-6-0030 test If a port has been mirror destination port, this port should not be allowed to join in link aggregation group."
)
class TestMirrorSession {
  ...
  @Test(
    "If a port has been mirror destination port, this port should not be allowed to join in link aggregation group."
  )
  private async testConfig() {
    let sessionId = await this.addDstMonitorSession(1, this.portName1);
  ...
  private async addDstMonitorSession(
    ...
    await this.topo.dut.exec`
      > configure terminal
      > monitor session ${sessionId} destination interface ${portName}
      > end
    `;
    return sessionId;
  }
